...
def updateTargets():
    ...
    player_list = [{"name":row['Name'], "target": row['Target']} for index, row in df.iterrows()]
...
def updateOtherData():
    ...
    player_list = [{"name":row['name'], "position": row['position'], "year_ended": row["year_end"]} for index, row in df.iterrows()]
    for player in player_list:
        name = player['name']
        position = player['position']
        year_end = int(player['year_ended'])
        print(name, position, year_end)
        collection.update({"Name": name}, {"$set": {"Position": position, "Year Ended": year_end}})
